...
import { useDeleteWorkout, useWorkouts, WORKOUT_QUERY_KEYS, WorkoutRecord } from "@/shared/api/workouts";
...
export const WorkoutsList = ({ selectedDate }: WorkoutsListProps) => {
  ...
  return (
    <section className="flex flex-col gap-y-1 flex-1">
      <SwipeableList>
        {workouts?.map((x) => (
          <SwipeableListItem key={x.id}
            trailingActions={trailingActions(x.id)}
          >
